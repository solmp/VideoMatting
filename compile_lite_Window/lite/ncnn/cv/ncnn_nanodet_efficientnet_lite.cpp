...
#include "ncnn_nanodet_efficientnet_lite.h"
#include "lite/utils.h"

using ncnncv::NCNNNanoDetEfficientNetLite;

NCNNNanoDetEfficientNetLite::NCNNNanoDetEfficientNetLite(const std::string &_param_path,
...
void NCNNNanoDetEfficientNetLite::resize_unscale(const cv::Mat &mat, cv::Mat &mat_rs,
...
void NCNNNanoDetEfficientNetLite::transform(const cv::Mat &mat_rs, ncnn::Mat &in)
{
  // BGR NHWC -> BGR NCHW
...
void NCNNNanoDetEfficientNetLite::detect(const cv::Mat &mat, std::vector<types::Boxf> &detected_boxes,
...
void NCNNNanoDetEfficientNetLite::generate_points(unsigned int target_height, unsigned int target_width)
{
...
void NCNNNanoDetEfficientNetLite::generate_bboxes(const NanoLiteScaleParams &scale_params,
                                                  std::vector<types::Boxf> &bbox_collection,
...
void NCNNNanoDetEfficientNetLite::generate_bboxes_single_stride(const NanoLiteScaleParams &scale_params,
                                                                ...
                                                                std::vector<types::Boxf> &bbox_collection)
{
  ...
  for (unsigned int i = 0; i < num_points; ++i)
  {
    ...
    box.x1 = max(0.f, x1);
    box.y1 = max(0.f, y1);
    box.x2 = min(img_width, x2);
    box.y2 = min(img_height, y2);
    box.score = cls_conf;
    box.label = label;
...
void NCNNNanoDetEfficientNetLite::nms(
