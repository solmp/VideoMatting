...
using ncnncv::NCNNPFLD;

NCNNPFLD::NCNNPFLD(const std::string &_param_path,
...
void NCNNPFLD::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  cv::Mat mat_rs;
...
void NCNNPFLD::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
  ...
  extractor.extract("output", landmarks_norm); // c=1,w=106*2,h=1
